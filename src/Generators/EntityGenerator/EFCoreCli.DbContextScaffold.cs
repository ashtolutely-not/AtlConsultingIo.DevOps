    ...
    public record struct DbContextScaffoldCommand
    {
        ...
        public DbContextScaffoldCommand WithTableNames( List<string> tableNames )
        {
            ...
            foreach( var tbl in tableNames )
                list.Add( string.Join( TextUtils.WhitespaceChar, Args.TableName, QualifiedName(tbl) ) );
        ...
        {
            _args.Clear();
            _args.Add( string.Join( TextUtils.WhitespaceChar, ContextNameArg , _configuration.ContextName ) );
            _args.Add( string.Join( TextUtils.WhitespaceChar, Args.EntityClassDirectory , _configuration.EntitiesOutDirectory.SurroundWithDoubleQuotes() ) );
            _args.Add( string.Join( TextUtils.WhitespaceChar, Args.EntityClassNamespace , _configuration.EntitiesNamespace ) );
            _args.Add( string.Join( TextUtils.WhitespaceChar, Args.DbContextClassDirectory , _configuration.ContextOutDirectory.SurroundWithDoubleQuotes() ) );
            _args.Add( string.Join( TextUtils.WhitespaceChar, Args.DbContextClassNamespace , _configuration.ContextNamespace ) );
            ...
            if( !_tableArgs.Any())
                _args.Add( string.Join( TextUtils.WhitespaceChar, Args.SchemaName , _configuration.Schema ) );
        }
        void setFlags()
        {
            _flags.Clear();
